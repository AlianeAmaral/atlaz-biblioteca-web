...
const saveLoan = async () => {
  ...
  try {
    ...
    const payload = {
      ...form.value,
      bookCode: Number(form.value.bookCode), // Garante que vai como número, não string
      // O input date já guarda como "YYYY-MM-DD", o que o LocalDate do Java ama.
    };

    console.log("Enviando para o Java:", payload); // Verifique o formato no console!
...
<template>
  <h1 class="text-2xl font-bold p- bg-teal-700 text-white">Cadastrar Novo Empréstimo</h1>
  ...
  <div class="p-8 max-w-2xl text-black">
    <form @submit.prevent="saveLoan" class="bg-white p-8 rounded-2xl shadow-lg flex flex-col gap-6">
      ...
      <div class="flex flex-col">
        ...
        <input v-model="form.startDate" type="date" required class="p-2 border rounded-lg outline-none focus:ring-2 focus:ring-lime-700" />
      </div>

      <div class="flex flex-col">
        <label class="font-bold mb-1">Data de Devolução</label>
        <input v-model="form.endDate" type="date" required class="p-2 border rounded-lg outline-none focus:ring-2 focus:ring-lime-700" />
      </div>
      ...
      <div class="flex flex-col">
        <label class="font-bold mb-1">Status</label>
            ...
            required 
            class="p-2 border rounded-lg outline-none bg-white focus:ring-2 focus:ring-cyan-700 cursor-pointer">         
